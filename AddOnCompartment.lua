...
C_Timer.After(1, function()                                                             -- wait to run this, so we can get the db
    if (WOW_PROJECT_ID == WOW_PROJECT_MAINLINE) then                                    -- If on Retail (this is only supported on 10.x and up
        ...
        if db.showAddOnCompartmentButton ~= nil and db.showAddOnCompartmentButton then  -- Check against a setting to enable this (if set AND true)
                ...
                func = function(button, menuInputData, menu)
                    ...
                    elseif menuInputData.buttonName == "RightButton" then               -- if a right click
                        ...
                        if db.soloMode ~= nil then                                      -- if solo mode is set
                            ...
                            DF:UpdateAllFrames()                                        -- update frames
