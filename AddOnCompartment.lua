...
--[[
    NOTE :: The addon compartment is registered on addon_loaded. The function for right click needs the profile for the character.
    This is why we regrab the profile. Anything needing a specific profile needs the profile grabbed on execution!
--]]

C_Timer.After(1, function()                                                               -- wait to run this, so we can get the db
    if (WOW_PROJECT_ID == WOW_PROJECT_MAINLINE) then                                      -- If on Retail (this is only supported on 10.x and up
        local gdb = DF:GetGlobalDB()                                                      -- get local DB Settings
        if gdb.showAddOnCompartmentButton ~= nil and gdb.showAddOnCompartmentButton then  -- Check against a setting to enable this (if set AND true)
