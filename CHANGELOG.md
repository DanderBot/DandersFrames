...
* (Auras) Filter presets now start with passive "noise" auras unticked — mastery procs and auto-applied side effects. Everything stays listed in the Filter Designer, one click to enable.
...
* (Profiles) Refreshed the default profile's look — new installs and profile resets only; existing settings are untouched. Frames sit tighter, buff borders are on (4 buffs shown, 2 per row), Missing Buffs is on by default, pet frames got a cleaner style, and lots of small icon, text and highlight positions were polished. The Big/External Defensive buff filters are no longer on by default — the defensive icons already cover those buffs, and having both duplicated them.
