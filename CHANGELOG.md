...
* (Aura Designer) **Groups can now colour the frame itself.** Every group card gets a Frame Effects section: a border, health bar colour, background colour, name text colour and health text colour that light up whenever any buff in the group is on the unit — no need to set them up buff by buff. Each one is off until you enable it, and a Priority slider decides which effect wins when more than one wants the same part of the frame (a single buff's own effect wins a tie). Available on both buff tabs, for hand-picked groups and filter groups alike.
...
* (Aura Filters) The buff bar's default filters are now Healing, Raid Cooldowns, Offensive Cooldowns and Power Externals. Raid Buffs is no longer on by default for new profiles — if you already have it on, it stays on. The three new ones are switched on for everyone once; turn any of them back off if you'd rather not see them.
* (Aura Filters) **New "Offensive Cooldowns" preset.** Track big damage cooldowns on your group — Avatar, Combustion, Metamorphosis, Voidform, Trueshot and 40 more across every class. Handy for timing Bloodlust or a Power Infusion to land in someone's burst window.
* (Aura Filters) Tank Cooldowns picks up Celestial Infusion and Sundering Roar, and Movement picks up Hover and Death Charge, which were all missing.
* (Aura Filters) Preset spell lists refreshed — new entries across Healing, External Defensives, Consumables and Trinkets & Items, and several spells that can appear under a second spell ID are now matched properly.
* (Aura Filters) **New "Tank Cooldowns" preset.** Tank abilities were missing from the built-in filters — Bone Shield, Vampiric Blood, Shield Wall, Metamorphosis, Celestial Brew and the rest of every tank's kit are now included. Big cooldowns also stay in Defensives; the short rotational buffs are listed but off by default, so tick the ones you want.
...
* (Aura Filters) **Fixed a very large spell ID breaking the filter you added it to.** An ID above the game's maximum — `2222222222`, say — was accepted, and the filter then wouldn't draw and its Delete button stopped responding until you clicked away to another filter and back. Oversized IDs are now refused as you type them, with the usual "Enter a valid spell ID." message. `1111111111` is under the limit and is still accepted. A filter that already picked one up will display and can be deleted normally.
* (Test Mode) **Fixed the preview showing the wrong icons — the same spell repeated, and artwork that didn't match its own tooltip.** Introduced in alpha 15. Each sample icon's art, position and tooltip could end up belonging to three different spells, so what you saw while styling wasn't what your frames would actually show. The preview matches again.
...
* (Frames) **Fixed the frame border being invisible.** It was sitting underneath the health bar, so at full health you saw none of it and at half health you saw half. Introduced during this alpha; it had never shipped this way.
