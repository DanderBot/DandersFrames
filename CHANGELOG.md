...
* (Bars) Fixed a hairline gap that could show between a "Match Health Bar Width" resource bar and the frame border at small border sizes (UI-scale dependent) — the bar's ends now anchor to the same frame corners the border does, so the two can't round apart.
