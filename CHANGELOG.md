...
* (Auras) **New: Refresh Highlight (pandemic).** Buff and debuff icons can show a cue while an aura is inside its refresh window — the stretch near the end where recasting it wastes nothing, so a HoT or DoT can be refreshed without clipping any of its remaining time. Choose a coloured **Tint** over the icon or a **Border** around it, set the colour and opacity, and optionally make it flash. Enable it in the new "Refresh Highlight" section on the Buffs and Debuffs pages, and on every Aura Designer icon, square and bar indicator. The game works the window out itself and drives the cue, so it stays exact in combat with nothing read or timed on our side. This sits alongside Expiry Alerts rather than replacing them — an expiry alert says "this is about to fall off" at a threshold you pick; the refresh highlight says "recasting now costs you nothing". Requires the current game build; the controls say so if your client is older.
...
* (Click Casting) **Turning Click Casting off now actually stops the hover binds.** Switching it off and declining the reload left every hover binding live and still swallowing keypresses while the settings screen said it was off. *(Reported by allie.)*
...
* (Pet Frames) Fixed the three pet tracks (party, raid and arena) resurrecting and overwriting one another, pets keeping a stale anchor when the game reassigned their unit, and arena pet settings needing a reload to take effect. Pet updates that land during combat are now deferred instead of dropped.
