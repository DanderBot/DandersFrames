...
* (Dispel) The Blizzard-rendered dispel overlay shows its full health-bar gradient again on 12.1 (Blizzard and Hybrid sources) — 12.1 replaced the attribute that enables the gradient, so only the small dispel icon was rendering. Also fixed a Lua error when opening Edit Mode with the overlay active (a game bug in the private-aura watcher under Edit Mode's sample data; the overlay now suspends during Edit Mode and rebuilds after).
