...
* (Aura Designer) **New Expiry Alert "Border" mode.** Alongside Custom Text and Glyph, a placed icon/square's Expiry Warning can now show a coloured **frame** when the aura is about to expire. It matches the icon size automatically (with an Inset to nudge it in or out) and comes in several styles — three frame thicknesses **or a solid Fill that tints the whole icon** — with an Opacity slider, and its colour can be a fixed pick **or** follow the same **Color by Time** breakpoints as your duration text — so the frame (or tint) reddens as the aura runs down, in step with the numbers. Fully combat-safe (it's driven render-side, no reliance on reading the remaining time).
