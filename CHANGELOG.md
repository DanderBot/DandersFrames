...
* (Aura Designer) **The Aura Designer now works for every spec** — it was previously limited to healers and Augmentation Evokers. Any spec can place and style indicators, and the spec dropdown is now grouped by class with a search box.
* (Aura Designer) **New: Filter Groups.** Place an entire filter on the frame as its own icon row. Add one from the Layout Groups tab, link any filters (built-in or your own custom ones), and every matching buff appears in a compact, self-arranging row — position, icon size, growth direction and icon cap are all yours to set. Linked filters stay in sync: changes you make in the Filter Designer apply to the group immediately, and buffs shown by a group are hidden from the buff bar while "Hide Duplicate Buffs" is on.
* (Aura Designer) The spell picker is now backed by the addon's full spell database — spells are grouped by class, searchable by name, and any spell can be added by its ID.
* (Aura Designer) Every indicator now has a show/hide eye, so you can turn one off without deleting it. Hidden indicators return their buff to the buff bar.
* (Aura Designer) The editor header has been condensed to a single row, and the Sound Alerts settings now live under the Global tab.
* (Profiles) Custom filters linked in the Aura Designer now travel with profile exports, just like the ones picked on the Aura Filters page.
* (Auras) **The Aura Blacklist has been retired**, replaced by the new filter system. Any buffs you had blacklisted will show again — to hide specific buffs, disable them in the Filter Designer or adjust your filter choices on the Aura Filters page. Individual debuffs can't be hidden on this version of the game. The old blacklist page now explains this and links to the replacements.
* (Auras) **New: Debuff Filters.** The debuff bar is now filtered by categories, found on the Aura Filters page — debuff filters were previously marked as a 12.1 roadmap item and are now live. Pick and choose: Boss Debuffs (applied by dungeon and raid bosses), Role Debuffs (ones the game flags as important for your role), Priority Debuffs (ones the game flags as high priority), Crowd Control (stuns, roots, incapacitates and the like), Raid Debuffs (other debuffs the game flags for raid frames), and Dispellable Debuffs (debuffs that can be dispelled — by you specifically, or by anyone). Prefer the old behaviour? "Show All Debuffs" still shows everything.
* (Auras) The "All Dispellable" debuff filter mode now actually filters — it previously showed every debuff, dispellable or not.
* (Auras) New "Hide Long Debuffs" filter (Aura Filters): hides debuffs whose total duration exceeds a threshold — e.g. a ten-minute tavern debuff. A "Keep important debuffs" option keeps boss, role and priority debuffs visible no matter how long they last.
* (Auras) Debuff filter preferences have been reset to new defaults as part of this rework.
* (Profiles) A settings page's Copy to Raid/Party, Sync and Reset Page buttons no longer touch settings that belong to a different page — e.g. resetting the Debuffs page no longer wipes your Aura Filters choices, and the Buffs/Debuffs pages no longer copy the click-through toggles owned by the Integrations page.
...
* (Interface) **Border animations are now entirely DF's own.** The bundled glow library is gone — every effect is drawn and driven by DF. Four new DF effects join the set: **DF Chase**, **DF Flash**, **DF Pixel** and **DF Proc**, alongside the existing DF Pulsate, DF Dash and Blink. The old library-backed Proc and Comet, plus Wipe, Ripple, Segment Reveal and Sides Only, are retired. Animations are available on the Defensive and Missing-Buff icons, Targeted Spells and Personal Targeted (including their Highlight variants), and Aura Designer indicators.
* (Auras) **Animated borders are back for Defensive and Missing-Buff icons.** Border animations were marked as a 12.1 limitation during the rework, but DF's own approach — DF-drawn border textures animated by a shared external driver, rather than Blizzard's aura-button glow — works on the container icons too, so the full DF animation set (Pulsate, Chase, Dash, Flash, Pixel, Proc, Blink) is available on them. Under the hood the animation driver is now shared (one timer for all animated borders instead of one each), so it's lighter across the board.
...
* (Aura Designer) Filter group and debuff group boxes in the editor preview now show example icons — greyed out to mark them as samples — sized and arranged to your icon size, spacing, growth and per-row settings, so you can style a group without guessing. Filter groups sample real spells from their linked filters; debuff groups show representative icons for the selected categories.
* (Aura Designer) Filter groups and debuff groups can now be styled like placed indicators, through the same collapsible **Appearance**, **Border**, **Duration Text** and **Stack Count** sections the indicator cards use — including the animated border styles and the cooldown swipe toggle. The example icons in the editor preview wear your styling live as you edit — border, duration text and stack count samples included. Existing groups look exactly as before until you change something.
...
* (Internal) **The pre-12.1 aura pipeline is fully removed.** Every aura surface — buff/debuff rows, defensives, missing buffs, dispel overlay, Aura Designer and all test-mode previews — now runs exclusively on the game's 12.1 aura engine; the old scanning, caching and hand-painted rendering code (over 11,000 lines) is gone. No behaviour change is intended beyond the entries above: if any aura display behaves differently than before, please report it.
* (Dispel) Fixed the Top/Bottom/Left/Right Edge gradient styles rendering as health-shaped fragments instead of a full-length strip after "On Current Health Only" (a Full Frame option) had been active — the health clipping stuck to the shared gradient bar across the style switch, on live frames and in test mode alike. This is a long-standing issue that predates the 12.1 rework.
* (Auras) Fixed a doubled gap between filter blocks on the 12.1 aura rows when more than one buff/debuff filter is enabled — the boundary between two filters' icons rendered at twice the configured spacing. Rows are now uniformly spaced throughout.
* (Auras) **Centered growth works again on 12.1** — buff, debuff and defensive icon rows set to grow from the center were rendering side-grown (the game's new flow layout has no centering concept, and it was previously marked as a 12.1 limitation). The rows now stay centered on their anchor as icons appear and disappear — live, in combat. The growth control is also clearer: the dropdown is now labelled **Grow** (was "Direction") with a **From Center** option (was "Center"), since that value describes a behaviour — growing outward both ways from the anchor — rather than a single direction. The Defensive Icon page's layout section also drops the outdated "(Direct Mode)" naming.
* (Test Mode) **Test mode now previews buffs, debuffs, defensive icons and missing buffs through the real 12.1 aura engine** — the rows on test frames are the actual container rows, laid out, bordered, fonted and animated exactly as they render live, wearing the addon's curated test icons (no more approximate hand-painted copies). The Buffs/Debuffs count sliders apply live, cooldown swipes run, dispel-type rings colour correctly, and hovering an icon shows its real spell tooltip. The defensive preview shows curated defensive spells (three on tanks, one on healers, matching the old preview shape) with live-true styling, and the missing-buff preview shows the real badge strip — one badge per tracked buff wearing its actual spell icon — instead of a single placeholder icon. The dispel overlay preview shows the game's dispel palette on the full art exactly as it renders live — including while dragging a slider — and turning the preview off now clears every part of the overlay (edge gradients and the darken layer used to linger). One side effect of the game's preview mechanism: while test mode is open, the game's own aura displays temporarily show sample data — everything restores the moment test mode closes.
* (Auras) The debuff "Color by Dispel Type" ring is now a flat square line matching the icon's own border instead of the game's rounded ring art, which clashed with square borders. It follows the debuff Border Thickness setting exactly at any icon size, so with Dispel Border Inset at 0 it sits precisely on your border — reading as "the border takes the dispel colour" — and negative insets keep the outward-halo look.
* (Borders) Fixed switching a border's style from Gradient back to Solid sometimes leaving the gradient stuck until a /reload. It hit the aura icon borders (buff/debuff/defensive/missing-buff) — their fast-path border widgets skipped the gradient-state clear on the assumption they could never carry one; the clear now runs whenever a gradient was actually painted. The frame border was unaffected.
* (Profiles) Fixed resetting a profile to defaults not sticking for the Aura Filters: after the next /reload the buff/debuff filters flipped back to an old preset (All Debuffs off, Big/External Defensives on). Two 2024-era one-time filter resets re-applied themselves whenever their bookkeeping was wiped by a profile reset; they've been removed — upgraded profiles already carry their result, and fresh or reset profiles get the current defaults directly.
* (Profiles) Fixed "Reset Profile to Defaults" (and `/df reset`) leaving the Aura Designer and Text Designer untouched — their configurations live in a shared, profile-level preset library that the per-mode reset never cleared, so edited designer presets survived a full reset. A reset now rebuilds both designer libraries to their defaults (the Default / Party / Raid presets) and re-applies them to live frames; any custom presets you created are cleared, as expected for a full profile reset.
* (Text Designer) The default Text Designer layout — name, current/max health, and status text — is now a proper baseline that new and reset profiles carry directly, instead of being rebuilt on load by the legacy migration. This fixes the Text Designer going blank until the next /reload after a profile reset, and means the legacy migration now only runs for profiles genuinely upgrading from before the Text Designer. The default health text stays current/max, matching what the built-in health text always showed.
* (Text Designer) The Texts and Text Groups tabs now show a live count of their enabled elements (e.g. "Text Groups (1)"), so it's obvious at a glance where your text lives — in particular that the default current/max health sits under Text Groups rather than Texts. The count updates as you add, remove, enable or disable elements.
* (Interface) The settings window now uses **DF Roboto SemiBold** as its default font instead of the game's Friz Quadrata. New profiles get it, and existing profiles still on the old default are switched over once — if you had deliberately chosen Friz Quadrata, just pick it again and it'll stick. This only affects the DandersFrames settings panel; in-game frame text is unchanged (use the Text Designer / Global Fonts for that).
* (Integrations) Removed the **Click-Through Icons** options (buff / debuff / defensive / targeted-spell). On 12.1 the aura icons are always click-through by design — the game's protected aura buttons can't intercept clicks, and DF forces it as well — so the toggles no longer did anything. Aura tooltips are still controlled from the Tooltips page. Any leftover values in imported or upgraded profiles are simply ignored.
* (Class Power) Removed the **Class Power Pips** display — the per-point pips (Holy Power, Chi, Combo Points, Soul Shards, Arcane Charges, Essence) that could be shown on your own frame — and its settings page, along with the matching Test Mode toggle. Any leftover values in imported or upgraded profiles are simply ignored.
* (Performance) Disabling the Blizzard party/raid frames now *fully* shuts them down rather than just hiding them. The default frames used to be kept alive (they were once a source of aura data, which 12.1 killed off) — now, when disabled, the Raid Utility manager, the frame containers and their events are properly unregistered and parked on a hidden frame, so the whole Blizzard raid/party system stops doing background work. Because that's a one-way shutdown, toggling **Disable Blizzard Party/Raid Frames** or **Show Side Menu** now prompts for a UI reload to take full effect (keeping the Side Menu falls back to the lighter hide, since that menu *is* the manager).
* (Dispel) **The Dispel Overlay is now one unified system on 12.1, driven by the game's new aura engine** — and it covers boss debuffs (private auras) natively, which no addon-drawn overlay could do before. The old Off / DandersFrames / Blizzard / Hybrid source selector is gone: there is a single Enable toggle (existing settings migrate — any non-Off source counts as enabled), and the separate "Blizzard Overlay" settings section is retired with it. The overlay is coloured by the game's own dispel palette, applied by the game engine — one overlay at a time exactly like the game's frames, switching type automatically and deterministically (your own casts first, then the oldest debuff). The old per-type custom colour pickers are retired with the rework: the game engine owns the colour on 12.1, and keeping a custom-coloured variant meant a second overlay mechanism with different behaviour for little gain. Everything else is yours to style — the border, edge glow, gradient position/size/opacity, blend mode, darken, "on current health only", the dispel-type icons and the pulse animation — live, in combat, with zero aura reads. Dispellable bleeds are included natively. Also fixed a Lua error when opening Edit Mode with the overlay active.
* (Auras) Opening WoW's Edit Mode no longer floods the 12.1 aura rows with random sample icons — the rows hide while Edit Mode is open and restore (with a refresh) when it closes. Edit Mode swaps the game's entire aura data layer onto sample data, and there is no way for an addon to keep real auras through it; a proper styled preview during Edit Mode is planned with the native test-mode work.
* (Profiles) Refreshed default-profile values (new installs / profile resets only — existing settings are untouched): tighter frame spacing, buff borders on, small buff/debuff position and text offset nudges, and the name text sits slightly higher. Also refreshed: Missing Buffs on by default; pet frames use the DF Minimalist bar texture with a health-gradient fill, a visible border and larger name/health text; the combat status icon moved above the frame (clear of the role icon) at a slightly larger scale; the leader icon nudged up; the raid-role icon shows as an icon rather than text; the aggro-highlight inset is flush (0); the personal targeted-spell highlight pulses slightly faster; and Text Designer defaults tweaked (name size 13 / max length 10, current-max health text size 12). Buffs now show 4 at a time, 2 per row; the debuff dispel ring sits flush on the icon's border (inset 0); the Targeted Spell and Personal Targeted highlight animations are now DF Proc with the intro flash off; and the Text Designer's name, health-group and status text offsets were nudged. The Big Defensive / External Defensive buff filters are no longer on by default — on 12.1 a buff matching several filters shows one icon per match, so the old defaults duplicated defensive buffs on the buff bar; the defensive icons already cover them.
* (Profiles) Fixed a handful of border settings silently reverting on profile export/import and on Copy between Party and Raid. The border animation's **Hide Intro Flash** toggle (Defensive Icon, Missing Buff, Targeted Spell and Personal Targeted, plus their Highlight variants) and the Defensive Icon / Missing Buff **border Colour Source** were never registered as real settings — the options wrote them and the frames read them, but they carried no default and were absent from the export list, so any import or party↔raid copy quietly dropped them back to default. They're now carried like every other border setting.
* (Test Mode) **Rebuilt the Quick Presets** (Static / Combat / Healer / Full). They only ever set 11 of the panel's 19 toggles, so the other eight — Show Pets, Heal Prediction, Reduced Max Health, Text Designer, Aura Designer, Animate Targeted List, Selection and Aggro — simply kept whatever value they happened to have. "Full" was never full, the Highlights section stayed empty on *every* preset, and "Static" wasn't a reproducible baseline (it left anything it didn't know about switched on). Presets are now declarative and exhaustive: each one sets every toggle, so they're reproducible and round-trip cleanly, and applying one repaints every test surface so it lands immediately instead of waiting for you to click the checkbox by hand. Static is the frame at rest, Combat adds health movement and threat, Healer adds the healing-decision layers (absorbs, incoming heals, reduced max health, defensives), and Full is genuinely everything. The party-only Targeted List toggles are no longer written into the raid profile, and the long-dead `testShowIcons` setting (merged into the single Icons toggle) is gone.
* (Test Mode) Fixed two party-only features leaking into the **raid** preview. The **Targeted List** demo bars appeared during raid test whenever the *party* profile had them enabled — the live path bails on `IsInRaid()`, but a raid preview isn't a real raid, so that gate never fired. And **Personal Targeted** resolved the *party* profile inside the raid preview, so the raid panel's toggle was ignored and the preview rendered with the party profile's size, position and style — exactly the bug its db resolver was written to prevent, just in test mode rather than live. A raid preview now counts as raid for that resolver.
* (Test Mode) In **raid** test mode the **Targeted Spells**, **Targeted List** and **Animate Targeted List** toggles are now greyed out. Both are party-only features — the group cast detection is fingerprint-based and can't disambiguate targets in a raid, and the Targeted List doesn't run in raid at all — so those toggles previously looked available while doing nothing. They no longer count toward the section badge either, and the raid Quick Presets leave them alone. Personal Targeted is unaffected: it works in raid and stays enabled.
* (Boss Debuffs) **Boss debuffs are now part of the regular debuff row on 12.1.** WoW's new aura system routes boss debuffs — including private auras, which addons can't see — through the same display engine as everything else, so they appear in the debuff row automatically (with duration, stacks and dispel colouring like any debuff). The separate Boss Debuffs display is retired: keeping it would show every private aura twice. Its settings page has been removed with it.
* (Missing Buffs) **Missing Buffs now work on 12.1 — including in combat and Mythic+.** The old detection had to read everyone's buffs, which 12.1 seals; the feature now uses a display-only mechanism driven entirely by the game engine, so it never reads aura data at all. That makes it *more* reliable than before: no combat delay, no Mythic+ blackout, and no dependence on the shrinking list of readable spells. One behaviour change in manual multi-buff mode: every tracked buff that is missing shows its own icon side by side (the old "show only the first missing" pick required reading auras). The Icon Size, Scale, Anchor/Offset, Frame Level and Border settings all apply live; "Border Animation" and "Debug Mode" are marked in place (not applicable to the new mechanism). "Hide Raid Buffs from Buff Bar" is also enforced natively now — and holds in combat, which the old version couldn't do. Missing buffs now also show for NPC party members (e.g. follower dungeons) — raid buffs are castable on them, but the old detection couldn't check them.
* (Defensives) Fixed a defensive appearing twice when it counts as both a personal and an external defensive (e.g. Guardian Spirit) — the row now uses a single game-side defensive category, and sorts the way the game intends: the save someone else cast on the player first, own defensives last. Also removed a duplicated pair of Duration Offset sliders on the Defensive Icon page.
...
* (Aura Designer) Fixed layout groups stacking all their spells on one spot on live frames — grouped indicators arrange into their configured grid again. Each spell keeps its own spot in the row, so when a buff is missing its slot stays empty rather than the others sliding along (a limitation of the new aura engine).
* (Aura Designer) Adding spells to a layout group now uses the full searchable spell picker instead of a small dropdown — browse the spell list, search by name or add by ID, then click Icon or Square to drop that spell straight into the group. The picker stays open so you can add several in a row.
...
* (Aura Designer) Filter groups on the Other Buffs tab now have their own "Others Only" option: the group only shows buffs cast by someone else, and your own casts still show on the regular buff bar.
