...
* (Test Mode) **Test mode now previews buffs, debuffs, defensive icons and missing buffs through the real 12.1 aura engine** — the rows on test frames are the actual container rows, laid out, bordered, fonted and animated exactly as they render live, wearing the addon's curated test icons (no more approximate hand-painted copies). The Buffs/Debuffs count sliders apply live, cooldown swipes run, dispel-type rings colour correctly, and hovering an icon shows its real spell tooltip. The defensive preview shows curated defensive spells (three on tanks, one on healers, matching the old preview shape) with live-true styling, and the missing-buff preview shows the real badge strip — one badge per tracked buff wearing its actual spell icon — instead of a single placeholder icon. The dispel overlay preview now matches the selected Color Source: Game Colors previews the game's palette on the gradient and type icon exactly as it renders live (borders and intensity are custom-mode-only), and turning the preview off now clears every part of the overlay (edge gradients and the darken layer used to linger). One side effect of the game's preview mechanism: while test mode is open, the game's own aura displays temporarily show sample data — everything restores the moment test mode closes.
...
* (Dispel) **The Dispel Overlay is now one unified system on 12.1, driven by the game's new aura engine** — and it covers boss debuffs (private auras) natively, which no addon-drawn overlay could do before. The old Off / DandersFrames / Blizzard / Hybrid source selector is gone: there is a single Enable toggle (existing settings migrate — any non-Off source counts as enabled), and the separate "Blizzard Overlay" settings section is retired with it. A new **Color Source** choice sets how the overlay is coloured: **Game Colors** (default) uses the game's own dispel palette, applied by the game engine — one overlay at a time exactly like the game's frames, switching type automatically; **Custom Colors** brings back the per-type colour pickers with the full art — borders, edge glow and gradient intensity included (a unit carrying two different dispellable types can briefly show both overlays in this mode). Gradient position/size/opacity, blend mode, darken, "on current health only", the dispel-type icon settings and the pulse animation work in both — live, in combat, with zero aura reads. Dispellable bleeds are included natively; the custom Bleed colour keeps its opt-in (set it non-black). Settings that need type knowledge the game palette can't give are hidden while Game Colors is selected. Also fixed a Lua error when opening Edit Mode with the overlay active.
...
* Dispel Overlay: "Color Name Text" is not yet wired to the new overlay (marked in place). A unit with dispellable debuffs of two different types shows both type icons overlapped (rare; in Custom Colors mode both overlays can show).
