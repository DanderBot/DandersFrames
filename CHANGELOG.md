...
* (Test Mode) **Fixed the preview showing the wrong icons — the same spell repeated, and artwork that didn't match its own tooltip.** Introduced in alpha 15, by a change that reduced how much the preview allocates. The preview builds each sample icon in its own slot, and paints the artwork per slot while the game decides where each icon lands; the alpha-15 change numbered the artwork in the order icons were *created* instead, which is not the order they are laid out in. Position, tooltip and art all came apart from one another. The preview is back to one slot per icon, so what you see while styling matches what ships to your frames again.
...
* (Frames) **Fixed party and arena frames flipping to vertical when combat ends**, if a layout change was queued while you were fighting. Three places read a setting that nothing ever writes and so always took the vertical branch; two of them run at header creation and are corrected a moment later by the normal settings pass, but the third runs off the combat queue with nothing behind it to put the orientation back.
