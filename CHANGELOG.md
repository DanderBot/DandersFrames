...
* (Auras) **New "Order Applied" sort order.** Buffs and debuffs can now be sorted by the order they were applied, so an icon keeps its place for as long as the aura lasts instead of moving each time it refreshes — worth choosing if you track auras by position rather than by reading them. Available on the Aura Filters page for the buff and debuff bars, and on every Aura Designer group card. "My Auras First" doesn't apply to it (it is one fixed order), so it greys out while it's selected. The Sort Order lists now read Default → Time Remaining → Alphabetical → Order Applied rather than alphabetically.
...
* (Interface) **Targeted Spells has been removed from the menu.** The group/on-frame half of the feature depends on aura information 12.1 no longer exposes to addons. **Personal** targeted spells are unaffected and stay where they are. Nothing has been deleted yet — if a later PTR build restores what it needs, it can come back.
...
* (Interface) **Aura Filters and Filter Designer: clearer names and readable rows.** "Other Buffs" is now **Any Buff**, the buff sections are named for what they hold, each pool tab explains itself, and New Filter sits in the section it creates into. The two info banners now say what you can *do* on each side rather than defining one in terms of the other — buff filters are opt-in and editable, the debuff blacklist is opt-out and fixed by what Blizzard allows. Buttons on a hovered spell row no longer dissolve into the highlight: the row's own action button lights up with the row (it shares the row's click), while the info and remove buttons stay legible without being primed. Spell tooltips rise from the icon and name at your cursor, and the panels use the window's full height.
