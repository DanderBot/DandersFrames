...
* (Aura Designer) Icon and square indicators have refreshed defaults — **DF Roboto SemiBold** with a drop shadow, slightly larger duration text, the stack count tucked into the bottom-right corner, and the duration text coloured by time remaining (was Friz Quadrata, no shadow, centred stacks, static colour) — matching the buff and debuff bars. Indicators still on the untouched old defaults are updated automatically; anything you've already customised is left alone.
