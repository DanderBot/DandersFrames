...
* (Aura Designer) Filter groups and debuff groups now have an **Appearance** section on their card — border (including the animated border styles), duration text, stack count and cooldown swipe — so grouped icons can be styled like placed indicators. Existing groups look exactly as before until you change something.
