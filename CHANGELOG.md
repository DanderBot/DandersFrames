...
* (Click Casting) The game resets a unit frame's click registration whenever a roster shuffle reassigns its unit — which silently turned off cast-on-down for Blizzard frames until something reapplied the bindings. Click casting now detects exactly which frames the game touched and restores its bindings on them automatically (immediately out of combat, at combat end otherwise). (by Krathe)
