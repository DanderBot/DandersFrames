...
function AE:LoadExperiment(key)
    ...
    -- The row editor is INDEX-keyed and the rows it was opened on no longer exist —
    -- exactly AE:RemoveRow's hazard, one row set wider. Left open it would edit an
    -- experiment's row under the previous row's title, and every tick the user then
    -- changed would corrupt the measurement they loaded the experiment to take.
    if self._editor then self._editor:Hide() end

    -- ⚠ SetActive(true) rebuilds unconditionally, including when the tool is already
    -- on — so no Rebuild() here, and none for the builder panel either: AE:Rebuild
    -- redraws it (and the container count) before its own active check. A second call
    -- is not free: every rebuild tears down and recreates one container PER ROW PER
    -- UNIT (see the header), which is the one cost this tool has to stay honest about.
    self:SetActive(true)
