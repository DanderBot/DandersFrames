...
- A drag snaps to the **nearest** free zone whose rect comes within the **Snap distance** setting (Settings › Snapping, default 100, 0–400) of the dragged rect, measured edge to edge — zero while the two overlap. The distance is fixed, so a wide raid container and a single icon are pulled in from the same gap; ties between overlapping zones go to the larger overlap. The zone highlight uses the same radius, so a zone that lights up is a zone that will take the drop.
