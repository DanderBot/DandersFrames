...
| `UI:StyleFolderTab(btn, opts)` | The OTHER kind of tab: one that belongs to the panel under it. `StyleButton{ tab = true }` is the underline tab and switches which VIEW of a page you see; a folder tab sits ON a panel and says what that panel is SHOWING — selected tab full height in the panel's own fill with NO ring (so it joins it), the rest `setBack` px shorter, darker and outlined behind it. Two languages, so a page can carry both strips without them reading as one block. `opts`: `text`, `font`, `radius`, `setBack`, `activeFill` / `inactiveFill` / `border`, `onClick`, `tooltip`. Driven by `btn:SetActive(bool)` like every other tab; anchor it by calling `btn:SetWidth(w)` and `btn:SetFolderX(x)`, because it re-issues both its own vertical points on every state change |
...
| `po:SetTetherOverride(region)`, `po:ClearTetherOverride()` | Point the tether at a region that is COVERING the source. When a consumer draws an overlay across the surface its rows live on, every panel's outline goes on lighting a rect nobody can see — it lands on whatever the overlay is drawing in that slot and reads as a claim about the overlay's own content. The override moves the outline, the beam and the clip gate together (they are one rect by contract) and `Clear` puts back EXACTLY what was there — nil, a region, or a function — so the consumer never has to name it. ONE override, not a stack: installing twice replaces the target and keeps the first stash, so an open path that can run twice against a single close still restores. An `adopt` voids an outstanding override, since the panel then belongs to a different row |
| `UI:SetPopoutTetherOverride(region)`, `UI:ClearPopoutTetherOverride()` | The same across every panel this host currently has up. An overlay covers a SURFACE, not one panel, so this is the verb a consumer wants — and the open set is host state it cannot walk itself |
