...
-- Defined before UpdateAuraIconsDirect so it
...
function DF:UpdateAuraIconsDirect(frame, icons, auraType, maxAuras)
    ...
    local isDirect = true -- direct is the only aura source (4.6.1)
...
-- instead of the legacy per-tick icon render. Hidden dev toggle; 12.1-only;
-- OFF in test mode (which paints legacy icons directly).
...
function DF:UseFactoryForBuffs(frame, db)
    ...
    -- (ToggleBuffFactory). Hard-gated to 12.1 + not test mode. (The old auraSourceMode
    -- gate was dropped when the Blizzard source mode was removed — everything is Direct now.)
