...
L["Choose which filters are active here. Customise what they contain — enable or disable individual spells, build your own custom filters, or edit the debuff Blacklist — in the"] = true
...
L["Colors page"] = true
L["Colours for each dispel type, shared by the debuff-icon border and the dispel overlay. Turn them on from each feature's own page."] = true
...
L["Debuffs are more limited: Blizzard only lets addons filter them by category (boss, dispellable, crowd control, and so on), not by individual spell, so the Blacklist is their only per-spell control."] = true
...
L["Out of Range"] = true
L["Opacity"] = true
L["Opt-in buff filters — you choose which buffs show. Enable or disable spells in the built-in presets, or create custom filters, then turn them on from the Aura Filters page."] = true
...
L["Recommended: use the category filters instead of 'All Debuffs' - they cover the important debuffs without the clutter."] = true
...
L["Reset: %s"] = true
L["Recolour the overlay from your custom dispel-type colours instead of the game's. Colours are set on the Colors page. Reload to apply to auras already showing."] = true
...
L["Sets the duration-text colors used wherever Color by Time Remaining is enabled (buffs, debuffs, defensives and the Aura Designer)."] = true
L["Settings"] = true
...
L["Sort your own auras before other players'. The Default sort order already shows yours first."] = true
...
L["Texts"] = true
L["Texture"] = true
L["The Aura Blacklist has been retired and replaced by the filter system."] = true
L["The reverse of the opt-in buff filters above: instead of choosing what to show, you choose nuisance debuffs to hide from the debuff bar. Only debuffs the game keeps non-secret can be hidden."] = true
...
L["To hide specific buffs, disable them in the Filter Designer or change which filters your buff bar uses on the Aura Filters page. On this version of the game, individual debuffs cannot be hidden on party and raid frames."] = true
...
L["Use Custom Dispel Colors"] = true
