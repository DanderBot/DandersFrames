...
L["aura filter state (add a unit token to target one)"] = true
L["dispel overlay state (add a unit token)"] = true
L["container identity-gate dump"] = true
L["missing-buff layout-push dump"] = true
L["toggle missing-buff badge park logging"] = true
...
L["lock frame movers (also: unlock, lockraid, unlockraid)"] = true
...
-- Popup.lua strings.
-- ⚠ 42 keys in this block are ORPHANED: they belonged to WizardBuilder.lua, which
-- was deleted as dead code. Left in place deliberately — pruning them is the same
-- edit across all 11 locale files, which is its own pass. Of the wizard's keys only
-- "Back", "Cancel", "Next", "None", "Party" and "Raid" are still read (Popup.lua,
-- ClickCasting/UI/BindingEditor.lua, DesignerPresets.lua) — do NOT remove those.
L["Branch"] = true
L["Building: "] = true
