...
do
    ...
    -- ---- the four bands ----------------------------------------------
    -- ⚠ EACH AT ITS OWN COLUMN'S WIDTH. Content, Text and the Factory band fill
    -- column 1 and Icon column 2 -- Text is on the left for balance (see the page).
    ...
    -- which width that is.
    local BAND_COL = { contentBand = 1, iconBand = 2, textBand = 1, factoryBand = 1 }
...
do
    ...
    -- Four bands in two columns -- Content, Text and Factory left, Icon right --
    ...
    local c = PAGE:find("Add(textBand, nil, 1)", 1, true)
...
--
-- Four of the nine. Layout is the biggest of them -- five settings and a blurb, six children, the whole of the budget and still inside it.
-- ============================================================
print("-- Defensive Icon page: the rows that mount their pane on the plate")
do
    ...
    local BEHIND = { "borderMount", "filterMount", "durationMount", "stackMount", "durBarMount" }
