...
do
    ...
    -- ---- the three bands ----------------------------------------------
    -- ⚠ EACH AT ITS OWN COLUMN'S WIDTH. Content and Effects fill column 1,
    -- Appearance column 2 -- Effects is on the left for balance (see the page). A band
    ...
    local BAND_COL = { contentBand = 1, appearanceBand = 2, effectsBand = 1 }
...
do
    ...
    -- Three bands in two columns -- Content and Effects left, Appearance right --
    ...
    local b2 = PAGE:find("Add(appearanceBand, nil, 2)", 1, true)
    local b3 = PAGE:find("Add(effectsBand, nil, 1)", 1, true)
    ...
    for _, band in ipairs({ "contentBand", "appearanceBand", "effectsBand" }) do
...
--
-- Four of the eight. Content Types is NOT one: five ticks between two lines of prose is seven children in party mode, and only prose in raid.
-- ============================================================
print("-- Personal Targeted Spells page: the rows that mount their pane on the plate")
do
    ...
    local BEHIND = { "contentMount", "borderMount", "durationMount", "highlightMount" }
